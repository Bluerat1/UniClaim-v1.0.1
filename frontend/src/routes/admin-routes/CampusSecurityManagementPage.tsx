...
export default function CampusSecurityManagementPage() {
  ...
  const handleConfirmCollection = (post: Post, _status: "collected" | "not_available") => {
  ...
  const handleCollectionConfirmation = async (status: "collected" | "not_available", notes?: string) => {
    ...
    try {
      ...
      console.log(`🔑 Using admin user ID for campus security collection confirmation: ${currentUserId}`);
      ...
      if (status === "not_available") {
        ...
        console.log('Item deleted:', postToConfirm.title, 'Reason: Not available at Campus Security');

      } else {
        // Always update status and transfer ownership when "collected" is clicked
        await postService.updateCampusSecurityTurnoverStatus(postToConfirm.id, status, currentUserId, notes);
        ...
        console.log('Campus security collection ownership transferred successfully:', postToConfirm.title, 'New owner:', currentUserId);
      }

    } catch (error: any) {
      console.error('Failed to process campus security collection confirmation:', error);
      const errorMessage = status === "not_available"
  ...
  return (
    <div className="min-h-screen bg-gray-100 mb-13 font-manrope transition-colors duration-300">
      <MobileNavText title="Campus Security Management" description="Manage items turned over to Campus Security" />

      {/* Header Section */}
      <div className="pt-4 px-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-800 mb-2">Campus Security Management</h1>
          <p className="text-gray-600">
            Manage all found items that have been turned over to Campus Security, including collection confirmations and status updates
          </p>
        </div>
      </div>
