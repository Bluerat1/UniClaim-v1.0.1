...
export const postService = {
  ...
  async createPost(
    ...
    try {
      ...
      console.log(
        `⚡ [PERF] Post ID generation: ${(postIdEnd - postIdStart).toFixed(
          2
        )}ms`
      );
      ...
      console.log(
        `📸 [PERF] Image upload (${postData.images.length} images): ${(
          imageUploadEnd - imageUploadStart
        ).toFixed(2)}ms`
      );
      ...
      console.log(
        `🧹 [PERF] Data sanitization: ${(sanitizeEnd - sanitizeStart).toFixed(
          2
        )}ms`
      );
      ...
      if (sanitizedPostData.turnoverDetails?.turnoverDecisionAt) {
        sanitizedPostData.turnoverDetails.turnoverDecisionAt =
          serverTimestamp();
      }
      ...
      console.log(
        `📋 [PERF] Post data preparation: ${(prepareEnd - prepareStart).toFixed(
          2
        )}ms`
      );
      ...
      console.log(
        `💾 [PERF] Firestore save: ${(dbSaveEnd - dbSaveStart).toFixed(2)}ms`
      );

      // Invalidate relevant caches after successful post creation
      cacheInvalidation.invalidatePostsByType("all");
      console.log(
        `🔄 [CACHE] Invalidated post caches after creating post ${postId}`
      );

      const endTime = performance.now();
      const totalTime = endTime - startTime;
      console.log(
        `✅ [PERF] Post creation completed successfully in ${totalTime.toFixed(
          2
        )}ms`
      );
  ...
  - Total: ${totalTime.toFixed(2)}ms`);
      ...
      const totalTime = endTime - startTime;
      console.error(
        `❌ [PERF] Post creation failed after ${totalTime.toFixed(2)}ms:`,
        error
      );
  ...
  async _sendPostNotificationsInBackground(
    postId: string,
    post: Post,
    creatorId: string
  ): Promise<void> {
    ...
    (async () => {
      ...
      try {
        ...
        const creatorName = creatorData
          ? `${(creatorData as any).firstName || ""} ${
              (creatorData as any).lastName || ""
            }`.trim()
          : "Someone";
        ...
        console.log(
          `🔔 [PERF] Background notifications sent: ${(
            notificationEnd - notificationStart
          ).toFixed(2)}ms`
        );
      } catch (notificationError) {
        console.error(
          "❌ [PERF] Error sending background notifications for post:",
          postId,
          notificationError
        );
      }
  ...
  async createPostWithConcurrentUpload(
    ...
    console.log(
      `🚀 [CONCURRENT] Starting concurrent post creation for user ${creatorId}`
    );
    ...
    try {
      ...
      const postIdEnd = performance.now();
      console.log(
        `⚡ [CONCURRENT] Post ID generation: ${(
          postIdEnd - postIdStart
        ).toFixed(2)}ms`
      );
      ...
      if (postData.images && postData.images.length > 0) {
        console.log(
          `🖼️ [CONCURRENT] Starting concurrent upload of ${postData.images.length} images`
        );
      ...
      const imageUploadEnd = performance.now();
      console.log(
        `📸 [CONCURRENT] Image upload completed: ${(
          imageUploadEnd - imageUploadStart
        ).toFixed(2)}ms`
      );
      ...
      const prepareEnd = performance.now();
      console.log(
        `📋 [CONCURRENT] Post data preparation: ${(
      ...
      const dbSaveEnd = performance.now();
      console.log(
        `💾 [CONCURRENT] Firestore save: ${(dbSaveEnd - dbSaveStart).toFixed(
          2
        )}ms`
      );
      ...
      const totalTime = endTime - startTime;
      console.log(
        `✅ [CONCURRENT] Post creation completed successfully in ${totalTime.toFixed(
          2
        )}ms`
      );
  ...
  - Total: ${totalTime.toFixed(2)}ms`);
      ...
      const totalTime = endTime - startTime;
      console.error(
        `❌ [CONCURRENT] Post creation failed after ${totalTime.toFixed(2)}ms:`,
        error
      );
      ...
      if (postId) {
        try {
          ...
          console.log(
            "⚠️ [CLEANUP] No images to clean up - error occurred before upload"
          );
        } catch (cleanupError) {
  ...
  getPostsByType(type: "lost" | "found", callback: (posts: Post[]) => void) {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const filteredPosts = posts.filter(
        (post) => post.status !== "resolved" && post.isHidden !== true
      );
  ...
  async getPostById(postId: string): Promise<Post | null> {
    try {
      ...
      // Fetch from database
      console.log(
        `🔍 [CACHE] Post ${postId} not in cache, fetching from database`
      );
  ...
  async cleanupOrphanedConversationsForFinalStatusPosts(): Promise<{
    cleanedPosts: number;
    totalConversationsDeleted: number;
  }> {
    try {
      console.log(
        "🔄 Starting cleanup of orphaned conversations for posts already marked as final status..."
      );
      ...
      console.log(
        `📋 Found ${postsSnapshot.docs.length} posts in final status that may need conversation cleanup`
      );
      ...
      for (const postDoc of postsSnapshot.docs) {
        ...
        try {
          ...
          if (conversationsSnapshot.docs.length > 0) {
            console.log(
              `🧹 Post ${postId} (${postData.title}) has ${conversationsSnapshot.docs.length} orphaned conversations - cleaning up`
            );
            ...
            console.log(
              `✅ Cleaned up ${conversationsSnapshot.docs.length} conversations for post ${postId}`
            );
          }
        } catch (error) {
          console.warn(
            `⚠️ Failed to cleanup conversations for post ${postId}:`,
            error
          );
      ...
      console.log(
        `🎯 Orphaned conversation cleanup completed: ${cleanedPosts} posts cleaned, ${totalConversationsDeleted} conversations deleted`
      );
      return { cleanedPosts, totalConversationsDeleted };
    } catch (error: any) {
      console.error("❌ Failed to cleanup orphaned conversations:", error);
      throw new Error(
        `Failed to cleanup orphaned conversations: ${error.message}`
      );
    }
  },
  ...
  async updatePostStatus(
    ...
    try {
      ...
      if (
        ...
        console.log(
          `🗑️ Post marked as ${status}, deleting all related conversations for post: ${postId}`
        );
        await this.deleteConversationsByPostId(postId);
        console.log(
          `✅ Successfully deleted conversations for ${status} post: ${postId}`
        );
      }
    } catch (error: any) {
    ...
    try {
      ...
      if (status === "confirmed") {
        ...
        if (adminDoc.exists()) {
          ...
          console.log(
            `📸 Final profile picture URL: ${osaAdminData.profilePicture}`
          );
        } else {
          console.warn(
            `⚠️ Admin document not found for user ID: ${adminUserId}`
          );
      ...
      const notificationRecipientId =
        postData?.turnoverDetails?.originalFinder?.uid || originalCreatorId;
      ...
      if (status === "confirmed") {
        ...
        if (
          ...
          notificationRecipientId !== confirmedBy
        ) {
          try {
            ...
            const adminName = adminData
              ? `${adminData.firstName || ""} ${
                  adminData.lastName || ""
                }`.trim() || "an administrator"
              : "an administrator";

            const notificationTitle = "Item Received";
            const notificationBody = `Your item "${
              postDoc.data()?.title || "item"
            }" has been received by ${adminName}.`;
            ...
            console.log(
              `📬 Sent receipt confirmation notification to user ${notificationRecipientId}`
            );
          } catch (notifError) {
            console.error(
  ...
  async updateCampusSecurityTurnoverStatus(
    ...
    try {
      const updateData: any = {
        "turnoverDetails.turnoverStatus":
          status === "collected" ? "transferred" : "not_received",
        "turnoverDetails.confirmedBy": confirmedBy,
        "turnoverDetails.confirmedAt": serverTimestamp(),
        updatedAt: serverTimestamp(),
      };
      ...
      if (status === "collected") {
        ...
        if (adminDoc.exists()) {
          ...
          adminData = {
            ...
            studentId: "", // Don't show admin ID
            profilePicture:
              adminUserData.profilePicture ||
              adminUserData.profileImageUrl ||
              DEFAULT_PROFILE_PICTURE,
            role: "admin", // Ensure role is set to admin
          };

          console.log(
            `📸 Final profile picture URL: ${adminData.profilePicture}`
          );
        } else {
          console.warn(
            `⚠️ Admin document not found for user ID: ${adminUserId}`
        ...
        try {
          console.log(
            `🔄 Starting conversation update for post ${postId} with status ${status}`
          );
          ...
          const conversationsSnapshot = await getDocs(conversationsQuery);
          console.log(
            `🔍 Found ${conversationsSnapshot.docs.length} conversations for post ${postId}`
          );

          if (!conversationsSnapshot.empty) {
            console.log(
              `🔄 Updating ${conversationsSnapshot.docs.length} conversations with new post creator`
            );
            ...
            const updatePromises = conversationsSnapshot.docs.map(
              async (convDoc) => {
                ...
                console.log(
                  `  - Current postCreatorId: ${conversationData.postCreatorId}`
                );
                console.log(
                  `  - Current participants keys:`,
                  Object.keys(conversationData.participants || {})
                );

                // Get admin user data to add to participants
                console.log(
                  `🔍 Fetching admin data for user ID: ${confirmedBy}`
                );
                const adminDoc = await getDoc(doc(db, "users", confirmedBy));
                const adminUserData = adminDoc.data();
                ...
                if (adminDoc.exists() && adminUserData) {
                  ...
                  console.log(
                    `✅ Created admin participant data:`,
                    adminParticipantData
                  );
                } else {
                  console.warn(
                    `⚠️ Admin user document not found for ${confirmedBy} (exists: ${adminDoc.exists()}, hasData: ${!!adminUserData})`
                  );
                  // Fallback if admin data not found
                  adminParticipantData = {
                    firstName: "System",
                ...
                if (originalCreatorId && participants[originalCreatorId]) {
                  ...
                  console.log(
                    `🔄 Removed campus security user ${originalCreatorId} from conversation ${convDoc.id}`
                  );
                }
                ...
                console.log(
                  `✅ Updated conversation ${convDoc.id} with new post creator: ${confirmedBy} and admin participant data (campus security user replaced)`
                );
              }
            );
          ...
          console.error(
            "❌ Failed to update conversations with new post creator:",
            conversationError
          );
      ...
      console.log(
        `🔍 Updated user: ${updatedData?.user?.firstName} ${updatedData?.user?.lastName}`
      );
      console.log(
        `🔍 Updated turnoverStatus: ${updatedData?.turnoverDetails?.turnoverStatus}`
      );
      console.log(
        `🔍 Updated turnoverAction: ${updatedData?.turnoverDetails?.turnoverAction}`
      );
      console.log(
        `🔍 Updated createdAt: ${
      ...
      if (status === "collected") {
        ...
        if (
          ...
          try {
            ...
            const notificationBody = `Your item "${
              postDoc.data()?.title || "item"
            }" has been collected from Campus Security by ${adminName}.`;
            const notificationData = {
              userId: notificationRecipientId,
              type: "claim_update",
              postId: postId,
            ...
            console.log(
              `📬 Sent collection confirmation notification to user ${notificationRecipientId}`
            );
          } catch (notifError) {
            console.error(
              "Failed to send collection confirmation notification:",
              notifError
            );
            // Don't fail the whole operation if notification fails
          }
      ...
      throw new Error(
        error.message || "Failed to update campus security collection status"
      );
    }
  },
    ...
    try {
      ...
      if (updates.images) {
        ...
        if (deletedImages.length > 0) {
          console.log(
            `🗑️ Deleting ${deletedImages.length} removed images from Cloudinary:`,
            deletedImages
          );
          ...
          for (const imageUrl of deletedImages) {
            try {
              ...
              if (publicId) {
                console.log(
                  `🗑️ Deleting removed image: ${imageUrl
                    .split("/")
                    .pop()} (Public ID: ${publicId})`
                );
                await cloudinaryService.deleteImage(publicId);
                console.log(
                  `✅ Successfully deleted removed image: ${imageUrl
                    .split("/")
                    .pop()}`
                );
              } else {
                console.warn(
                  `⚠️ Could not extract public ID from removed image URL: ${imageUrl}`
                );
              }
            } catch (error) {
              console.error(
                "❌ Failed to delete removed image:",
                imageUrl,
                error
              );
  ...
  async deletePost(
    ...
    try {
      ...
      if (postData.handoverDetails?.handoverItemPhotos) {
        postData.handoverDetails.handoverItemPhotos.forEach(
          (photo: { url: string }) => {
      ...
      if (postData.claimDetails?.evidencePhotos) {
        postData.claimDetails.evidencePhotos.forEach(
          (photo: { url: string }) => {
            if (photo.url) allImagesToDelete.push(photo.url);
          }
        );
      ...
      if (hardDelete) {
        ...
        if (allImagesToDelete.length > 0) {
          console.log(
            `🗑️ Deleting ${allImagesToDelete.length} total images from Cloudinary`
          );
          // Run in background without awaiting
          Promise.all(
  ...
  async deleteConversationsByPostId(postId: string): Promise<void> {
    try {
      ...
      console.log(
        `🔄 Auto-rejecting pending requests for ${conversationsSnapshot.docs.length} conversations`
      );
        ...
        try {
          ...
          if (messagesSnapshot.docs.length > 0) {
            ...
            for (const messageDoc of messagesSnapshot.docs) {
              ...
              if (
                messageData.messageType === "claim_request" &&
                messageData.claimData?.status === "pending"
              ) {
                console.log(
                  `🔄 Auto-rejecting pending claim request in conversation ${conversationId}, message ${messageDoc.id}`
                );
              ...
              if (
                ...
                console.log(
                  `🔄 Auto-rejecting pending handover request in conversation ${conversationId}, message ${messageDoc.id}`
                );
                ...
                rejectedRequests.push({
                  ...
                  postTitle:
                    messageData.handoverData.postTitle || "Unknown Post",
                });
                ...
                console.log(
                  `✅ Auto-rejected handover request: ${messageDoc.id}`
                );
              }
            }
          }
          ...
          console.warn(
            `Failed to process conversation ${conversationId} for request rejection:`,
            error.message
          );
      ...
      if (rejectedRequests.length > 0) {
        console.log(
          `📨 Sending notifications for ${rejectedRequests.length} auto-rejected requests`
        );

        // Group requests by user to send batch notifications
        const userNotifications = new Map<
          string,
          Array<(typeof rejectedRequests)[0]>
        >();
        ...
        for (const [userId, userRequests] of userNotifications) {
          try {
            const requestTypeText =
              userRequests[0].requestType === "claim" ? "claim" : "handover";
            const postTitles = userRequests.map((r) => r.postTitle).join(", ");

            await notificationSender.sendNotificationToUser(userId, {
              type: "claim_response",
              title: `${userRequests.length} ${requestTypeText}${
                userRequests.length > 1 ? "s" : ""
              } auto-rejected`,
              body: `Your ${requestTypeText} request${
                userRequests.length > 1 ? "s" : ""
              } for "${postTitles}" ${
                userRequests.length > 1 ? "have" : "has"
              } been automatically rejected because the post${
                userRequests.length > 1 ? "s were" : " was"
              } deleted.`,
              data: {
                status: "rejected",
            ...
            console.log(
              `✅ Sent auto-rejection notification to user ${userId} for ${
                userRequests.length
              } ${requestTypeText}${userRequests.length > 1 ? "s" : ""}`
            );
          } catch (notificationError) {
            console.warn(
              `Failed to send auto-rejection notification to user ${userId}:`,
              notificationError
            );
      ...
      console.log(
        `🗑️ Starting image cleanup for ${conversationsSnapshot.docs.length} conversations`
      );
      ...
      for (const convDoc of conversationsSnapshot.docs) {
        ...
        try {
          ...
          if (messagesSnapshot.docs.length > 0) {
            console.log(
              `🗑️ Processing ${messagesSnapshot.docs.length} messages in conversation ${conversationId}`
            );
            ...
            for (const messageDoc of messagesSnapshot.docs) {
              ...
              try {
                ...
                if (messageImages.length > 0) {
                  console.log(
                    `🗑️ Found ${messageImages.length} images in message ${messageDoc.id}`
                  );
                  allImageUrls.push(...messageImages);
                }
              } catch (imageError: any) {
                console.warn(
                  `Failed to extract images from message ${messageDoc.id}:`,
                  imageError.message
                );
          ...
          console.warn(
            `Failed to process conversation ${conversationId} for image extraction:`,
            error.message
          );
          // Continue with other conversations even if one fails
        }
      ...
      if (allImageUrls.length > 0) {
        console.log(
          `🗑️ Attempting to delete ${allImageUrls.length} total images from Cloudinary`
        );
        ...
        try {
          ...
          if (imageDeletionResult.success) {
            console.log(
              `✅ Successfully deleted ${imageDeletionResult.deleted.length} images from Cloudinary`
            );
          } else {
            console.warn(
              `⚠️ Image deletion completed with some failures. Deleted: ${imageDeletionResult.deleted.length}, Failed: ${imageDeletionResult.failed.length}`
            );
          }
        } catch (imageError: any) {
          console.warn(
            "Failed to delete images from Cloudinary, but continuing with database cleanup:",
            imageError.message
          );
      ...
      for (const convDoc of conversationsSnapshot.docs) {
        ...
        try {
          ...
          throw new Error(
            `Failed to process conversation ${conversationId}: ${error.message}`
          );
        }
      }
      ...
      console.log(
        `✅ Successfully deleted ${conversationsSnapshot.docs.length} conversations and their messages`
      );
    } catch (error: any) {
  ...
  async activateTicket(postId: string): Promise<void> {
    try {
      ...
      if (!postData.movedToUnclaimed && postData.status !== "unclaimed") {
        throw new Error(
          "Post is not in unclaimed status and cannot be activated"
        );
      }
      ...
      console.log(
        `Post ${postId} activated and moved back to active status with status: ${statusToRestore}`
      );
    } catch (error: any) {
  ...
  async flagPost(
    ...
    try {
      ...
      console.log(
        `✅ Post ${postId} flagged by user ${userId} for reason: ${reason}`
      );
    } catch (error: any) {
  ...
  async hidePost(postId: string): Promise<void> {
    try {
      ...
      try {
        ...
        console.error(
          "❌ Failed to send hide notification:",
          notificationError
        );
        // Don't fail the operation if notification fails
      }
  ...
  async unhidePost(postId: string): Promise<void> {
    try {
      ...
      try {
        ...
        console.error(
          "❌ Failed to send unhide notification:",
          notificationError
        );
        // Don't fail the operation if notification fails
      }
  ...
  async cleanupHandoverDetailsAndPhotos(
    postId: string
  ): Promise<{ photosDeleted: number; errors: string[] }> {
    try {
      console.log(
        `🧹 Starting cleanup of handover details and photos for post ${postId}...`
      );
      ...
      if (
        handoverDetails.handoverItemPhotos &&
        Array.isArray(handoverDetails.handoverItemPhotos)
      ) {
      ...
      if (handoverDetails.handoverRequestDetails?.itemPhotos) {
        handoverDetails.handoverRequestDetails.itemPhotos.forEach(
          (photo: any) => {
            if (photo.url) {
              imageUrls.push(photo.url);
            }
      ...
      if (imageUrls.length > 0) {
        console.log(
          `🗑️ Deleting ${imageUrls.length} handover photos from Cloudinary`
        );

        try {
          const { deleteMessageImages } = await import(
            "../../utils/cloudinary"
          );
          ...
          console.log(
            `✅ Cloudinary cleanup result: ${result.deleted.length} deleted, ${result.failed.length} failed`
          );

          if (result.failed.length > 0) {
            errors.push(
              ...result.failed.map(
                (fail: any) => `Failed to delete ${fail.url}: ${fail.error}`
              )
            );
          }
      ...
      console.log(
        `✅ Handover cleanup completed: ${photosDeleted} photos deleted`
      );
      ...
      return {
        photosDeleted: 0,
        errors: [error.message || "Failed to cleanup handover details"],
      };
    }
  },

  // Cleanup claim details and photos when reverting a resolution
  async cleanupClaimDetailsAndPhotos(
    postId: string
  ): Promise<{ photosDeleted: number; errors: string[] }> {
    try {
      console.log(
        `🧹 Starting cleanup of claim details and photos for post ${postId}...`
      );
      ...
      // Add evidence photos
      if (
        claimDetails.evidencePhotos &&
        Array.isArray(claimDetails.evidencePhotos)
      ) {
      ...
      if (claimDetails.claimRequestDetails?.evidencePhotos) {
        claimDetails.claimRequestDetails.evidencePhotos.forEach(
          (photo: any) => {
            if (photo.url) {
              imageUrls.push(photo.url);
            }
      ...
      if (imageUrls.length > 0) {
        console.log(
          `🗑️ Deleting ${imageUrls.length} claim photos from Cloudinary`
        );
      ...
      console.log(
        `✅ Claim cleanup completed: ${photosDeleted} photos deleted`
      );
      return { photosDeleted, errors };
    } catch (error: any) {
      ...
      return {
        photosDeleted: 0,
        errors: [error.message || "Failed to cleanup claim details"],
      };
    }
  },
  ...
  async updateTurnoverStatus(
    ...
    try {
      console.log(
        `🔄 Updating turnover status for post ${postId} to ${status}`
      );
      ...
      if (
        status === "confirmed" &&
        postData.turnoverDetails?.turnoverAction === "turnover to OSA"
      ) {
        updateData["turnoverDetails.turnoverStatus"] = "transferred";
        console.log(
          `🔄 Auto-updating status to "transferred" for OSA turnover item`
        );
      }
